#include <aricc_depth_vision/depth_register.h>

namespace aricc_depth_vision {

  void DepthRegister::onInit(){
    NODELET_INFO("Starting DepthRegister Nodelet");
    ConnectionBasedNodelet::onInit();
    tf_buffer_.reset( new tf2_ros::Buffer );
    tf_.reset( new tf2_ros::TransformListener(*tf_buffer_) );
    pub_image_ = advertise<sensor_msgs::Image>(
      *pnh_, "output", 1);;
    pub_image_info_ = advertise<sensor_msgs::CameraInfo>(
      *pnh_, "output/camera_info", 1);;
    pnh_->param("approximate_sync", approximate_sync_, false);
    pnh_->param("queue_size", queue_size_, 5);
  }
  
  void DepthRegister::subscribe(){
    //NODELET_INFO("Subscribing ...");
    sub_image_.subscribe(*pnh_,      "input", 1);
    sub_depth_info_.subscribe(*pnh_, "input/depth_camera_info", 1);
    sub_rgb_info_.subscribe(*pnh_,   "input/rgb_camera_info", 1);
    if(approximate_sync_) {
      //NODELET_INFO("ASYN ...");
      async_ = boost::make_shared<message_filters::Synchronizer<ApproximateSyncPolicy> >(100);
      async_->connectInput(sub_image_, sub_depth_info_, sub_rgb_info_);
      async_->registerCallback(boost::bind(&DepthRegister::execute, this, _1, _2,_3));
    }
    else {
      //NODELET_INFO("SYN ...");
      sync_ = boost::make_shared<message_filters::Synchronizer<SyncPolicy> >(100);
      sync_->connectInput(sub_image_, sub_depth_info_, sub_rgb_info_);
      sync_->registerCallback(boost::bind(&DepthRegister::execute, this, _1, _2,_3));
    }
  }
  
  void DepthRegister::unsubscribe(){
    sub_image_.unsubscribe();
    sub_depth_info_.unsubscribe();
    sub_rgb_info_.unsubscribe();
  }

  void DepthRegister::execute(
    ...
    try{
      ...
      ROS_WARN("No TF from [%s] to [%s]",
        rgb_info_msg->header.frame_id.c_str(), 
        depth_info_msg->header.frame_id.c_str());
      return;
    ...
    registered_msg->header.stamp    = image_msg->header.stamp;
    registered_msg->header.frame_id = rgb_info_msg->header.frame_id;
    registered_msg->encoding        = image_msg->encoding;
    ...
    if (image_msg->encoding == sensor_msgs::image_encodings::TYPE_16UC1){
      convert<uint16_t>(image_msg, registered_msg, depth_to_rgb);
    }
    else if (image_msg->encoding == sensor_msgs::image_encodings::TYPE_32FC1){
      convert<float>(image_msg, registered_msg, depth_to_rgb);
    }
    else{
      NODELET_ERROR_THROTTLE(5, "Depth image has unsupported encoding [%s]", image_msg->encoding.c_str());
      return;
    }
    ...
    pub_image_.publish(registered_msg);
    pub_image_info_.publish(registered_info_msg);
  }

  template<typename T>
  void DepthRegister::convert(
...
PLUGINLIB_EXPORT_CLASS(aricc_depth_vision::DepthRegister,nodelet::Nodelet);
