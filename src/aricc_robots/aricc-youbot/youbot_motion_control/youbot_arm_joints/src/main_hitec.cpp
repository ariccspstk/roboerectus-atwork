...
protected:
  ...
  bool is_arm_pub_;
  double rate_;
  boost::mutex arm_mutex_, gripper_mutex_;
...
public:
  JointAction(std::string name) :
    ...
    pub_gripper_open_ =  nh_.advertise<std_msgs::Empty > 
      ("gripper_open", 1);
    pub_gripper_close_ =  nh_.advertise<std_msgs::Empty > 
      ("gripper_close", 1);
  ...
  void subscribe(){
    sub_arm_ = nh_.subscribe("/joint_states"  ,1,
      &JointAction::jointsCB,this);
    sub_gripper_ = nh_.subscribe("gripper_ready"  ,1,
      &JointAction::gripperCB,this);
  }
  ...
  void gripperCB(const std_msgs::Empty::ConstPtr& msg){
    boost::mutex::scoped_lock lock(gripper_mutex_);
    gripper_ready_msg_ = *msg;
  }
  ...
  bool isGripperReached( std::vector<double> goal){
    ...
    if(!is_gripper_pub_){
      ...
      if(goal.at(0) == 1 && goal.at(1) == 1)
        ...
        pub_gripper_close_.publish(msg);
      is_gripper_pub_ = true;
      return false;
    }
    //Checking the goal is reached or not
    //ros::Time now = ros::Time::now();
    //double time_diff = (now - gripper_ready_msg_.stamp).toSec();
    //if(fabs(time_diff) > 0.5) return false;
    sleep(2);
    return true;
  }
