    ...
    switch(task_.now){
      ...
      case kMoveFindObject:
        state = findItemAndContainer( task_.items, 
          best_item_, best_container_ );
        if( state == SUCCEEDED ){
          state = moveArmUpDownIk(best_container_, -1, false);
          if( state == SUCCEEDED ) task_.now = kStopBase;
        }
      break;
      */
      ...
      case kBaseAlignObject:
        basePose.x = 0.0;
        basePose.y = best_container_[0].position.y + offset_y_;
        basePose.theta = 0.0;
        if(isBaseMoveEnough(basePose.y)) task_.now = kStaticFindObjectBeforeGrasp;
        else{
          state = moveBase(basePose);
      ...
      */
      ...
      case kOpenGripperLoadTable:
        state = moveGripper("open");
        if(state == SUCCEEDED) task_.now = kArmGoUpAfterLoadTable;
        //else if(state == ABORTED) return ABORTED;
      break;
