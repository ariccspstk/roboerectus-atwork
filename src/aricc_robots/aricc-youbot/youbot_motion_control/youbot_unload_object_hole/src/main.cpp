...
public:
  ...
  std::string toString() const{
    switch(now){
      ...
      case kArmTwistBeforeLoadHole:  return "kArmTwistBeforeLoadHole";
...
public:
  ...
  typedef actionlib::SimpleActionClient<youbot_arm_ik::IkAction> AcArmIk;
...
protected:
  ...
  std::vector< aricc_vision_msgs::Object > best_holes_;
  aricc_vision_msgs::Object best_hole_;
  std::vector< std::string > best_items_;
  size_t best_slot_;
...
public:
  ...
  bool loadTables(){
    ...
    ROS_INFO("----------");
    ROS_INFO("Loading stations ...");
    XmlRpc::XmlRpcValue table_list;

    if( pnh_.getParam("precision_stations", table_list) ){
      ...
      if( table_list.getType() == XmlRpc::XmlRpcValue::TypeArray ){
        ...
        ROS_INFO("Loaded %lu stations",table_list_.size());
        return true;
      }
    }
  ...
  bool initAction(){
    ...
    pnh_.param("table_param_namespace", table_param_namespace_, std::string("/youbot_hole_detection/RotatedRectFinder/"));
  ...
  StateEnum twistArm( aricc_vision_msgs::Object object ){
    StateEnum state;
    ...
    double offset = cnt_arm_move_*0.02;
    pos.y += offset;
    state = moveArmIk(pos, ori,solution_);
    if(state == SUCCEEDED) {
      cnt_arm_move_++;
      sleep(2);
    }
  ...
  StateEnum moveArmIk( geometry_msgs::Point pos,
    ...
    if( client_arm_ik_->getState().toString() == "SUCCEEDED" ){
      is_arm_sent_goal_ = false;
      solution.clear();
      solution =  client_arm_ik_->getResult()->solution;
      return SUCCEEDED;
    }
  ...
  StateEnum moveArmUpDownIk(aricc_vision_msgs::Object object, 
    ...
    if(dir == 1) {
      pos.z += d;
      state = moveArmIk(pos, ori, solution_, move_arm);
      return state;
    }
    //move arm down
    if(dir == -1){
  ...
  StateEnum findHole( std::vector<std::string> items,
    ...
    std::vector< aricc_vision_msgs::Object >& best_holes ){
    boost::mutex::scoped_lock lock(object_mutex_);
    ...
    ros::Time now = ros::Time::now();
    std::vector< aricc_vision_msgs::Object > detected_holes;
    ...
    for( ; rit != object_msgs_.rend(); ++rit){
      ...
      for(std::vector< aricc_vision_msgs::Object >::iterator itt = rit->objects.begin(); itt != rit->objects.end(); ++itt){
         detected_holes.push_back(*itt);
      }
    }

    int holes_size = detected_holes.size();
    if( holes_size == 0 ){
    ...
    for(size_t i = 0; i < detected_holes.size(); ++i){
      str += detected_holes.at(i).name;
      str += " ";
    }
    str += "]";
    ...
    */
    ...
    for(std::vector< aricc_vision_msgs::Object >::iterator it = detected_holes.begin(); it != detected_holes.end(); ++it){
      for(std::vector<std::string>::iterator itt = items.begin(); 
        itt != items.end(); ++itt){
        if( it->name == *itt || 
            itt->rfind(it->name) != std::string::npos){
          cd_holes.push_back(*it);
          break;
        }
      }
    }
    ...
    /*
    str = "candidate holes:\n[";
    for(size_t i = 0; i < cd_holes.size(); ++i){
      str += cd_holes.at(i).name;
      str += " ";
    }
    str += "]";
    ROS_INFO("%s",str.c_str());
    */
    ...
    //Merge the same holes
    std::vector<aricc_vision_msgs::Object> temp_holes;
    temp_holes.clear();
    temp_holes.push_back(cd_holes[0]);
    for(std::vector< aricc_vision_msgs::Object >::iterator it = cd_holes.begin(); it != cd_holes.end(); ++it){
      bool same_name = false;
      for(std::vector< aricc_vision_msgs::Object >::iterator itt = temp_holes.begin(); itt != temp_holes.end(); ++itt){
        if(it->name == itt->name) same_name = true;
      }
      if(!same_name) temp_holes.push_back(*it);
    }
    best_holes.clear();
    best_items.clear();
    for ( std::vector< std::string >::iterator it=items.begin(); it!=items.end(); ++it){
      for(std::vector< aricc_vision_msgs::Object >::iterator itt = temp_holes.begin(); itt != temp_holes.end(); ++itt){
        std::string key = itt->name;
        std::size_t found = it->rfind(key);
        if(found != std::string::npos){
          best_items.push_back(*it);
          //Adjust angle of object to 0 or PI/2
          if( fabs(itt->orientation.z) <= aricc_utils::deg2Rad(5) )
            itt->orientation.z = 0.0;
          else if(fabs(itt->orientation.z- M_PI_2) <= aricc_utils::deg2Rad(5))
            itt->orientation.z = M_PI_2;
          best_holes.push_back(*itt);
        }
      }
    }
    ...
    for(std::vector< aricc_vision_msgs::Object >::iterator it = best_holes.begin(); it != best_holes.end(); ++it){
      ROS_INFO("best_hole: %s:[%.3lf,%.3lf,%.3lf]",
        it->name.c_str(),
        it->position.x, it->position.y,
        aricc_utils::rad2Deg(it->orientation.z));
    }
    for(std::vector< std::string >::iterator it=best_items.begin(); it!=best_items.end(); ++it){
      ROS_INFO("best_item: [%s]", it->c_str());
    }
    return SUCCEEDED;
  }
  ...
  size_t getTraySlot( std::vector<std::string> tray, 
    std::string item){
    for( std::vector<std::string>::iterator it = tray.begin(); it != tray.end(); ++it ){
      if(*it == item) return it - tray.begin();
    }
  }
    ...
    switch(task_.now){
      ...
      */
      ...
      case kStaticFindHole:
        state = findHole( task_.items, best_items_, best_holes_ );
        if(state == SUCCEEDED) task_.now = kBaseAlignHole;//kCheckArmIkBeforeUnload;
      ...
      case kBaseAlignHole:
        ...
        basePose.theta = 0.0;
        state = moveBase(basePose);
        if(state == SUCCEEDED) task_.now = kStaticFindHoleBeforeUnload;//kCheckArmIkBeforeUnload;
        else if(state == ABORTED) task_.now = kBaseMoveLeft;
      break;

      case kStaticFindHoleBeforeUnload:
        state = findHole( task_.items, best_items_, best_holes_ );//findHole( task_.items, best_hole_);
      ...
      case kCheckArmIkBeforeUnload:
        state = moveArmUpDownIk(best_holes_[0], -1, false);
        if(state == SUCCEEDED) 
          task_.now = kOpenGripperBeforeUnload;
      ...
      case kOpenGripperBeforeUnload:
        ...
        if(state == SUCCEEDED) {
          best_slot_ = getTraySlot( tray_state_,best_items_[0]);
      ...
      case kCloseGripperUnload:
        if(best_items_[0] == "S40_40_G" ||
           best_items_[0] == "S40_40_B" ||
           best_items_[0] == "M30")
      ...
      case kArmGoDownBeforeLoadHole:
        state = moveArmUpDownIk(best_holes_[0], -1);
        if(state == SUCCEEDED) task_.now = kOpenGripperLoadHole;
        //if(state == SUCCEEDED) task_.now = kArmTwistBeforeLoadHole;
        //else if(state == ABORTED) task_.now = kArmLookTable;
      break;

      case kArmTwistBeforeLoadHole:
        state = twistArm(best_holes_[0]);
      ...
      */
      ...
      case kUpdateBestList:
        if(best_holes_.size() != 0 &&
          best_items_.size() != 0){
          best_holes_.erase (best_holes_.begin());
          best_items_.erase (best_items_.begin());
        }
        ROS_INFO("Have %lu best holes",best_holes_.size());
        if(best_holes_.size() == 0 &&
           best_items_.size() == 0){
          ...
          task_.now = kCheckArmIkBeforeUnload;
      break;
  ...
  StateEnum isGoalValid( youbot_unload_object_hole::UnloadGoal goal){
    ...
    std::string str = "Final task items: [";
