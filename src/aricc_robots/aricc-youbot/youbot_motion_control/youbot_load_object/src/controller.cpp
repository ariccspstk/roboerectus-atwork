...
#include <youbot_load_object/ControllerConfig.h>
...
class YoubotLoadObjectController{
  public:
    typedef actionlib::SimpleActionClient<youbot_load_object::LoadAction> ActionClient;
    typedef youbot_load_object::ControllerConfig Config;

    YoubotLoadObjectController():nh_("~"),rate_(5){
      client_.reset( new ActionClient("youbot_load_object_server", true) );
      if(!client_->waitForServer(ros::Duration(5.0))){
        ROS_ERROR("youbot_load_object_server is not excuting");
      }
      srv_ = boost::make_shared <dynamic_reconfigure::Server<Config> > (nh_);
      dynamic_reconfigure::Server<Config>::CallbackType f =
        boost::bind (&YoubotLoadObjectController::configCallback, this, _1, _2);
      srv_->setCallback(f);
...
int main( int argc, char** argv ){
  ros::init( argc, argv, "youbot_load_object_controller" );
  YoubotLoadObjectController rt_;
  rt_.spin();
  return 0;
}
