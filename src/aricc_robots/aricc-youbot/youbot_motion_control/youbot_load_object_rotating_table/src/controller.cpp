...
#include <youbot_load_object_rotating_table/ControllerConfig.h>
...
class YoubotLoadObjectRotatingTableController{
  public:
    typedef actionlib::SimpleActionClient<youbot_load_object_rotating_table::LoadAction> ActionClient;
    typedef youbot_load_object_rotating_table::ControllerConfig Config;

    YoubotLoadObjectRotatingTableController():nh_("~"),rate_(5){
      client_.reset( new ActionClient("youbot_load_object_rotating_table_server", true) );
      if(!client_->waitForServer(ros::Duration(5.0))){
        ROS_ERROR("youbot_load_object_rotating_table_server is not excuting");
      }
      srv_ = boost::make_shared <dynamic_reconfigure::Server<Config> > (nh_);
      dynamic_reconfigure::Server<Config>::CallbackType f =
        boost::bind (&YoubotLoadObjectRotatingTableController::configCallback, this, _1, _2);
      srv_->setCallback(f);
    ...
    ~YoubotLoadObjectRotatingTableController(){
    }
  ...
  private:
    ...
    void loadObject(){
      youbot_load_object_rotating_table::LoadGoal goal;
...
int main( int argc, char** argv ){
  ros::init( argc, argv, "youbot_load_object_rotating_table_controller" );
  YoubotLoadObjectRotatingTableController rt_;
  rt_.spin();
  return 0;
}
