...
#include <youbot_load_object_rotating_table/LoadAction.h>
#include <youbot_load_object_rotating_table/ServerConfig.h>
...
public:
  ...
  std::string toString() const{
    switch(now){
      ...
      case kArmGoDownGrasp:         return "kArmGoDownGrasp";
      case kWaitObject:             return "kWaitObject";
...
public:
  ...
  typedef youbot_load_object_rotating_table::ServerConfig Config;
...
protected:
  ...
  actionlib::SimpleActionServer
    <youbot_load_object_rotating_table::LoadAction> action_;
  ...
  Trajectory armTrajectoryLookRotatingTable_; 
  ...
  youbot_load_object_rotating_table::LoadGoal goal_;
  youbot_load_object_rotating_table::LoadFeedback feedback_;
  youbot_load_object_rotating_table::LoadResult result_;
 
  ros::Subscriber sub_odom_, sub_object_, sub_beam_, sub_RTT_;
...
public:
  ...
  bool loadTables(){
    ...
    if( pnh_.getParam("rotating_stations", table_list) ){
  ...
  bool initAction(){
    ...
    pnh_.param<double>("sleep_delay", sleep_delay_, 0.0);
    ...
    pnh_.param<std::string>
     ...
     pnh_.param<std::string>
      ("rtt_topic", rtt_topic_, "youbot_2d_vision/rtt_detect_RTTImage/output");
  ...
  void subRTT(){
    sub_RTT_ = nh_.subscribe( rtt_topic_, 1, &LoadAction::RTTCB,this);
  }
  ...
  void configCallback(Config &config, uint32_t level){
    sleep_delay_ = config.delay;
    ROS_INFO("Set delay: %lf",sleep_delay_);
  }

  bool loadAllArmTrajectories(){
    if(!loadArmTrajectory("arm_trajectory_look_rotating_table",armTrajectoryLookRotatingTable_))
      return false;
  ...
  void RTTCB(const std_msgs::Header::ConstPtr& msg){
    boost::mutex::scoped_lock lock(rtt_mutex_);
    //ROS_INFO("Got rtt msg");
    rtt_msg_ = *msg;
  }
  ...
  StateEnum initArm(){
    ...
    //goal.points = armTrajectoryLookRotatingTable_;
  ...
  StateEnum moveArmUpDownIk(geometry_msgs::Point position, geometry_msgs::Point orientation, int dir, bool move_arm = true){
    StateEnum state;
    geometry_msgs::Point pos = position;
    pos.x += offset_x_;
    pos.y += offset_y_;
    //pos.z = offset_z_;

    geometry_msgs::Vector3 ori;
    ori.x = orientation.z + offset_roll_;
    ori.y = M_PI_2;
    ori.z = 0.0;
  ...
  StateEnum waitObject(){
    boost::mutex::scoped_lock lock(rtt_mutex_);
    ros::Time now = ros::Time::now();
    double time_diff = (now - rtt_msg_.stamp).toSec();
    if(fabs(time_diff) < 0.5) return SUCCEEDED;
    return ACTIVE;	
  }
  ...
  StateEnum taskStep(){
    ...
    geometry_msgs::Point pos, ori;
    switch(task_.now){
      case kArmLookTable:
        state = moveArmTrajectory(armTrajectoryLookTable_);
        //state = moveArmTrajectory(armTrajectoryLookRotatingTable_);
      ...
      */
      ...
      case kOpenGripperBeforeGrasp:
        ...
        if(state == SUCCEEDED) 
	{
         ROS_INFO("Delay: %lf",sleep_delay_);
	 sleep(sleep_delay_);
	 task_.now = kArmGoDownBeforeGrasp;
	}
        //else if(state == ABORTED) task_.now = kArmLookTable;
      break;

      case kArmGoDownBeforeGrasp:
        pos = best_objects_[0].position;
        ori.x = best_objects_[0].orientation.z + offset_roll_;
        ori.y = M_PI_2;
        ori.z = 0.0;
        pos.x = 0.0;
        pos.y = -0.05;
        pos.z =  offset_z_+0.05;
        state = moveArmUpDownIk(pos, ori, -1); 
        if(state == SUCCEEDED) task_.now = kWaitObject;
      ...
      case kWaitObject:
        state = waitObject();
        if(state == SUCCEEDED) task_.now = kArmGoDownGrasp;
        //else if(state == ABORTED) task_.now = kBaseMoveLeft;
      break;
      
      case kArmGoDownGrasp:
      ...
      case kCloseGripperGrasp:
        if(best_objects_[0].name == "F20_20_B" ||
           best_objects_[0].name == "F20_20_G" ||
  ...
  StateEnum isGoalValid( youbot_load_object_rotating_table::LoadGoal goal){
  ...
  void setAborted(youbot_load_object_rotating_table::LoadResult result, 
    std::string str = ""){
  ...
  void setSucceeded(youbot_load_object_rotating_table::LoadResult result){
  ...
  void executeCB(const youbot_load_object_rotating_table::LoadGoalConstPtr &goal_msg){
    goal_ = *goal_msg;
...
int main(int argc, char **argv) {
  ros::init(argc, argv, "youbot_load_object_rotating_table_server");
